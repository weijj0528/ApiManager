...
public class HttpTest {
    public static JSONObject demoTest(String url, JSONObject biz_param, String debugIsLogin) throws Exception {
        ...
        json.put("biz_module", param[0]);
        json.put("biz_method", param[1]);
        ...
        if (biz_paramLogin.optInt("isLogin") == 1) {
            ...
            String loginUrl = loginType == 0 ? "/account/login.do" : "/account/erpLogin.do";
            json.put("biz_param", biz_paramLogin);
            JSONObject httpPostJson = HttpClientUtils.httpPostJson(url.replace("/handle/control.do", loginUrl), json);
